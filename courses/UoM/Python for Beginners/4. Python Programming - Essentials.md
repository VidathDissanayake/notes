...
    <statement(s)>
```
`def` is the keyword that we use to inform Python of the new function we are creating. `<function_name>` is the identifier, and this should also follow the naming conventions. The component named `<parameter/s>` is optional. It is there to provide the parameters to be passed to the function, if any are needed. The indented <statement/s> compose the function body defining the functionality. We include the statements we need executed when the function is called , there.
